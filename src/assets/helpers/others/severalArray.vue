<template>
  <Combobox as="div" v-model="model" multiple>
    ...
    <div class="relative mt-1" ref="selectRef" tabindex="0">
      <ComboboxButton
        :class="[
          'w-full rounded-md border-0 bg-transparent py-1.5 px-3 text-mainText shadow-sm flex items-center relative',
      ...
      >
        <ComboboxInput
          :class="['flex-1 outline-none bg-transparent text-mainText text-base font-medium']"
          autocomplete="off"
      ...
      <ComboboxOptions
        ...
        :class="[
          'absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md py-1 shadow-xl ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm bg-main text-sm text-mainText',
        ]"
      >
        <ComboboxOption
        ...
        >
          <li
            :class="[
              'relative cursor-pointer select-none py-2 pl-3 pr-9 z-10 hover:bg-hoverAside hover:text-whiteText group',
              active ? 'bg-hover' : 'bg-main text-changeText',
            ]"
          >
            ...
            <span
              v-if="selected"
              :class="[
                'absolute inset-y-0 right-0 flex items-center pr-4 group-hover:text-whiteText',
                active ? 'text-changeText' : 'text-changeText',
              ]"
            >
